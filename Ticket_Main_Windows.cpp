...
//					and the totalNum less than maxNum
// ============================================
void Put(int id)
{
	while (1)					
...
int main()
{
	...
	SetThreadPriority(consumer1.native_handle(), THREAD_PRIORITY_HIGHEST);
	SetThreadPriority(consumer2.native_handle(), THREAD_PRIORITY_NORMAL);
	SetThreadPriority(consumer3.native_handle(), THREAD_PRIORITY_LOWEST);
	// ============== Block ended ==================================
	producer1.detach();
	producer2.detach();
	consumer1.detach();
